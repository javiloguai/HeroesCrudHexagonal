...
import com.hexagonal.heroestest.infraestructure.config.aspects.annotations.LogExecutionTime;
...
import com.hexagonal.heroestest.infraestructure.restapi.mappers.request.SuperHeroRequestMapper;
import com.hexagonal.heroestest.infraestructure.restapi.mappers.response.SuperHeroResponseMapper;
...
public class SuperHeroeController extends BaseController {
    ...
    private final SuperHeroesApplicationService superHeroesApplicationService;

    private final PagedResourcesAssembler<SuperHeroDomain> pagedResourcesAssembler;

    private final HeroPageModelAssembler heroPageModelAssembler;

    public SuperHeroeController(final SuperHeroesApplicationService superHeroesApplicationService,
            final PagedResourcesAssembler<SuperHeroDomain> pagedResourcesAssembler,
            final HeroPageModelAssembler heroPageModelAssembler) {
        this.superHeroesApplicationService = superHeroesApplicationService;
        this.pagedResourcesAssembler = pagedResourcesAssembler;
        this.heroPageModelAssembler = heroPageModelAssembler;
    }
    ...
    public ResponseEntity<List<HeroResponse>> getAllSuperHeroes() {
        ...
        List<SuperHeroDomain> allheroes = this.superHeroesApplicationService.getAllSuperHeroes();
        LOGGER.debug(
    ...
    public ResponseEntity<PagedModel<HeroResponse>> pageAllSuperHeroes(@ParameterObject final Pageable pageable) {
        ...
        Page<SuperHeroDomain> allheroes = this.superHeroesApplicationService.pageAllSuperHeroes(pageable);
        LOGGER.debug(
                "If it's necessary to pick apart the business objects from the response objects we could deal with a mapper here. " + "I'll will not repeat this or implement on the other endpoints, is just an example.");
        if (allheroes.isEmpty()) {
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    ...
    public ResponseEntity<List<HeroResponse>> getAllSuperHeroesByName(@RequestParam(required = true) String name) {
        LOGGER.debug("We can log whatever we need...");
        List<SuperHeroDomain> allheroes = this.superHeroesApplicationService.getAllSuperHeroesByName(name);
        LOGGER.debug(
                "If it's necessary to pick apart the business objects from the response objects we could deal with a mapper here. I'll will not repeat this or implement on the other endpoints, is just an example.");
        if (allheroes.isEmpty()) {
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    ...
    public ResponseEntity<PagedModel<HeroResponse>> pageAllSuperHeroesByName(@RequestParam(required = true) String name,
        ...
        LOGGER.debug("We can log whatever we need...");
        Page<SuperHeroDomain> allheroes = this.superHeroesApplicationService.pageAllSuperHeroesByName(name, pageable);
    ...
    public ResponseEntity<List<HeroResponse>> getAllSuperHeroesByPower(
        ...
        LOGGER.debug("We can log whatever we need...");
        List<SuperHeroDomain> allheroes = this.superHeroesApplicationService.getAllSuperHeroesBySuperPower(power);
        LOGGER.debug(
                "If it's necessary to pick apart the business objects from the response objects we could deal with a mapper here. I'll will not repeat this or implement on the other endpoints, is just an example.");
        if (allheroes.isEmpty()) {
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    ...
    public ResponseEntity<HeroResponse> getSuperHeroById(@PathVariable(value = "id", required = true) Long id) {
        LOGGER.debug("We can log whatever we need...");
        SuperHeroDomain hero = superHeroesApplicationService.findSuperHeroById(id);
        LOGGER.debug(
                "If it's necessary to pick apart the business objects from the response objects we could deal with a mapper here. I'll will not repeat this or implement on the other endpoints, is just an example.");
...
//    @PreAuthorize("hasAnyAuthority('ADMIN')")
    public ResponseEntity<HeroResponse> createSuperHero(@Valid @RequestBody final HeroRequest heroRequest) {
        ...
        SuperHeroDomain hero = superHeroesApplicationService.createSuperHero(
    ...
    public ResponseEntity<HeroResponse> updateSuperHero(@PathVariable(value = "id", required = true) Long id,
        ...
        SuperHeroDomain hero = superHeroesApplicationService.updateSuperHero(id,
                SuperHeroRequestMapper.INSTANCE.fromRequestToDto(heroRequest));
    ...
    public ResponseEntity<HeroResponse> addPowerToSuperHero(@PathVariable(value = "id", required = true) Long id,
        ...
        SuperHeroDomain hero = superHeroesApplicationService.addSuperPower(id, power);
    ...
    public ResponseEntity<HttpStatus> deleteSuperHero(@PathVariable("id") long id) {
        ...
        superHeroesApplicationService.deleteSuperHeroById(id);
